...
def tweet_random_image():
    ...
    for images in beta:
        if ".webm" not in images and ".mp4" not in images and ".gif" not in images and ".ini" not in images:
            if path_to_images + alpha + images not in used_paths_list["list"]:
                beta_refined.append(images)
                used_paths_list["list"].append(path_to_images + alpha + images)  # add full file name for duplicate file names in different directories
